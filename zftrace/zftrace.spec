Summary: Tracks and displays files used by a command
Name: zftrace
Version: 0.5-beta
Release: 1%{?dist}
License: BSD
Group: Development/Debuggers
URL: http://zenterio.com
Source: http://zenterio.com/%{name}-%{version}.tar.xz
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)

BuildRequires: libacl-devel, libaio-devel, time

%define zftrace64_arches ppc64 sparc64

%description
ftrace is a file use tracer, i.e. a debugging tool which prints out
a log of all the files used by a another process/program.
The program to be traced need not be recompiled for this, so you can
use it on binaries for which you don't have source.


%ifarch %{zftrace64_arches}
%package -n zftrace64
Summary: Tracks and displays system calls associated with a running process.
Group: Development/Debuggers

%description -n zftrace64
ftrace is a file use tracer, i.e. a debugging tool which prints out
a log of all the files used by a another process/program.
The program to be traced need not be recompiled for this, so you can
use it on binaries for which you don't have source.
...
This package provides the `zftrace64' program to trace 64-bit processes.
The `zftrace' program in the `zftrace' package is for 32-bit processes.
%endif
...
rm -f %{buildroot}%{_bindir}/zftrace-cp
...
%ifarch %{zftrace64_arches}
%{copy64} %{buildroot}%{_bindir}/zftrace %{buildroot}%{_bindir}/zftrace64
%endif
...
- New upstream release, work mostly by Dmitry V. Levin <ldv@altlinux.org>
  + exit/kill zftrace with traced process exitcode/signal (#105371);
...
- fix zftrace -f (Roland McGrath, #68994)
...
- zftrace 3.1 patches carried along for now.
